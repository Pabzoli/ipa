...
class _QuestionsPageState extends State<QuestionsPage>
  ...
  Future<void> _loadQuestions() async {
    try {
      ...
      if (widget.mode == QuizMode.asyncChallenge) {
        // Load the exact question IDs passed in — same order for both players
      ...
      setState(() {
        ...
        _errorMsg         = null;
        _eliminatedOptions =
            List.generate(_questions.length, (_) => {});
  ...
  void _selectOption(int index) {
    ...
    if (index == correct) {
      // In challenge mode, the Cloud Function transfers the bet to the winner.
      // We never add per-question score here — only weekly points always apply.
  ...
  void _moveNext() {
    ...
    if (_qIndex < _questions.length - 1) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) async {
        if (!mounted) return;
        if (widget.mode == QuizMode.asyncChallenge) {
          // Submit score to Firestore first, then navigate
          try {
            if (widget.isCreator) {
            ...
            if (!mounted) return;
            // Score submission failed — show error, don't leave user stuck
            ...
            // Try again on next frame rather than losing the score silently
  ...
  void _confirmQuit() {
    // Challenge quizzes cannot be abandoned — the bet is already locked in.
    // The score submission happens at the end of all 10 questions.
    if (widget.mode == QuizMode.asyncChallenge) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
